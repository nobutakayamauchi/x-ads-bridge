...
from bridge import BridgeError, execute
...
def main() -> int:
    ...
    try:
        ...
        _write_result("X Ads bridge blocked/failed", command, {"ok": False, "error": str(exc)})
        return 1
    except Exception as exc:
        _write_result("X Ads bridge unexpected failure", command, {"ok": False, "error": type(exc).__name__, "message": str(exc)})
        return 1
