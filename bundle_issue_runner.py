...
def main() -> int:
    try:
        ...
        if not title.startswith("[xbundle]"):
            raise RuntimeError("ignored: issue title must start with [xbundle]")
        if event_action != "opened":
            raise RuntimeError(
                "bundle operation actions require a newly opened issue; reopened issues are never accepted"
            )
        ...
        _write_result(
            "X Ads Bundle Operation unexpected failure",
